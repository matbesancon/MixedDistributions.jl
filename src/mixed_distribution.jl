...
function Dst.insupport(md::MixedDistribution, x)
    return minimum(md) <= x && x <= maximum(md)
end

"""
    Evaluates the probability density function of a mixed distribution.
    At a mass point, returns the associated mass probability, otherwise,
    returns the weighted sum of continuous densities at the point.
"""
function Dst.pdf(md::MixedDistribution, x)
    for idx in eachindex(md.mass_points)
        if md.mass_points[idx] ≈ x
            return md.mass_probs[idx]
        end
    end
    cont_point = 0.0
    for idx in eachindex(md.cont_dists)
        cont_point += (md.cont_weights[idx] * Dst.pdf(md.cont_dists[idx],x))
    end
    return cont_point
end

function Dst.cdf(md::MixedDistribution, x)
    cdf_point = 0.0
...
struct CDFException{D<:Dst.Distribution} <: Exception
	CDFException(::D) where {D<:Dst.Distribution} = new{D}() 
end
